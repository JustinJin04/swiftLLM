...
def marlin_gemm(A, B, s, workspace):
    C = torch.empty(A.shape[:-1]+(s.shape[1],), dtype=A.dtype, device=A.device)
...
# print(f"_perm: {_perm} \n"
#        f"_scale_perm: {_scale_perm} \n"
#        f"_scale_perm_single: {_scale_perm_single}")


def tile_swizzle_pack(w):
    # w: torch.int32 [k, n]
    # return: torch.int32 [k // 16, n * 2]
    k, n = w.shape
    tile = 16
...
def unpack_unswizzle_untile(q):
    # q: torch.int32 [k // 16, n * 2]
    # return: torch.int32 [k, n]
    k = q.shape[0] * 16
    n = q.shape[1] // 2
    tile = 16
    ...
    q = q.cpu().numpy().astype(np.int32)
    res = np.zeros((q.shape[0], q.shape[1]*8), dtype=np.int32)
    for i in range(8):
        res[:, i::8] = (q >> 4 * i) & 0x0F
    res = torch.from_numpy(res).to(q_dev)
    res = res.reshape((-1, inv_perm.numel()))[:, inv_perm].reshape(res.shape)
    w = res
    w = w.reshape((k // tile, n // tile, tile, tile))
    w = w.permute((0, 2, 1, 3))
    w = w.reshape((k, n))
...
class Layer(nn.Module):
    ...
    def pack(self, linear, scales):
        ...
        #     q |= res[:, i::8] << 4 * i
        # q = torch.from_numpy(q.astype(np.int32)).to(w.device)
        q = tile_swizzle_pack(w)
        assert torch.max(torch.abs(unpack_unswizzle_untile(q) - w)) == 0, "Unpacking did not yield the original weights."
